...
class ConfigTab(ttk.Frame):
    def __init__(self, parent) -> None:
        ...
        self.ip_ent = ttk.Entry(config_container)
        self.ip_ent.insert(0, config.get_ip())
        self.ip_ent.config(validate="key", validatecommand=v_ip)
        self.ip_ent.grid(row=0, column=1, sticky="w", padx=5, pady=5)
        ...
        self.cmd_ent = ttk.Entry(
            config_container, validate="key", validatecommand=v_num
        )
        self.cmd_ent.insert(0, str(config.get_cmd_port()))
        self.cmd_ent.grid(row=1, column=1, sticky="w", padx=5, pady=5)
        ...
        self.data_ent.grid(row=2, column=1, sticky="w", padx=5, pady=5)

        # Interval Spinner
        ttk.Label(config_container, text="UI Update Interval (ms):").grid(
            row=3, column=0, sticky="e", padx=5, pady=5
        )
        self.ui_spin = ttk.Spinbox(config_container, from_=10, to=500, increment=10)
        self.ui_spin.set(config.get_update_interval())
        self.ui_spin.grid(row=3, column=1, sticky="w", padx=5, pady=5)
        ...
        self.samples_ent.grid(row=2, column=1, sticky="w", padx=5, pady=5)
    ...
    def _update_settings(self):
        ...
        config.update_settings(
            ip, cmd_port, data_port, update_interval, samples_per_page
        )
